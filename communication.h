 ...
 */
  static Communication& getInstance(const __FlashStringHelper aApn[], const __FlashStringHelper aLogin[], const __FlashStringHelper aPassword[], const String& aServerName, const int aServerPort) {
    if (!pCommunication) {
 ...
 */
  String getIMEI() const {
    if (connectGSMGPRS(GSM_CONNECTION)) { 
      ...
      DEBUG(F("Error connecting GSM in ")); DEBUG(F(__FUNCTION__)); DEBUG('\n');
      return "";
    }
  }
...
// Pas sûr que ça serve à quelque chose.
    if (!client.connect(serverName.c_str(), serverPort)) {
      DEBUG(F("No HTTP connection in ")); DEBUG(F(__PRETTY_FUNCTION__)); DEBUG(F("!\n"));
      return false;
    }
 ...
 */
  Communication(const __FlashStringHelper aApn[], const __FlashStringHelper aLogin[], const __FlashStringHelper aPassword[], const String& aServerName, const int aServerPort) :
    modem(SerialGSM),
    GPRS_APN(aApn),
    GPRS_LOGIN(aLogin),
    GPRS_PASSWORD(aPassword),
    ...
    switch (aState) {
      ...
      case GPRS_CONNECTION :
        for (int i = 0; i < aRetry; ++i) {
          ...
          const String apn(GPRS_APN);
          const String login(GPRS_LOGIN);
          const String password(GPRS_PASSWORD);
          if (!modem.gprsConnect(apn.c_str(), login.c_str(), password.c_str())) {
